...
export function createRadioOpsApi(client){
  ...
  return {
    ...
    async listProfiles(){ return unwrap(await client.from('profiles').select('id,employee_id,display_name,department,email,role,is_active,approval_status,is_primary_manager,approved_at,approved_by,rejected_at,disabled_at,last_status_change_at,created_at').neq('approval_status','REMOVED').order('display_name',{ascending:true}),'Unable to load employees') || []; },
