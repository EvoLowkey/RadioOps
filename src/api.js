...
export function createRadioOpsApi(client){
  ...
  return {
    ...
    async listProfiles(){ return unwrap(await client.from('profiles').select('id,employee_id,display_name,department,role,is_active').eq('is_active',true).order('display_name',{ascending:true}),'Unable to load employees') || []; },
