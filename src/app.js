...
function renderSecureQr(container,radioId,token){
  ...
  card.innerHTML=`<div class="qr-label-brand">VALET RADIO HQ</div><h2>${escapeHtml(radioId)}</h2><div class="secure-qr-preview" aria-label="${escapeHtml(radioId)} secure QR"></div><small>SCAN TO CHECK OUT / RETURN</small>`;
  container.appendChild(card);
  const qrTarget=card.querySelector('.secure-qr-preview');
  if(typeof QRCode!=='function') throw new Error('QR renderer unavailable');
  new QRCode(qrTarget,{text:token,width:180,height:180,correctLevel:QRCode.CorrectLevel.M});
}
function showBarcodeLabel(radioId,token){currentDymoLabel={radioId,token};$('#qrLabelTitle').textContent=radioId;renderSecureQr($('#qrPrintArea'),radioId,token);$('#qrLabelDialog').showModal();}
async function rotateOneQr(radioId){
  if(!confirm(`Generate a new secure QR for ${radioId}? Any previous ${radioId} QR code label will stop working immediately.`))return;
  try{const result=await api.rotateRadioQrToken(radioId);showBarcodeLabel(radioId,result.token);showToast(`${radioId}: secure QR generated`);await loadData({quiet:true});}catch(err){showToast(humanError(err));}
}
async function generateAllQrLabels(){
  if(!confirm('Generate new secure QRs for ALL 40 radios? This immediately invalidates every previous radio QR code label. Continue only when you are ready to install the complete new label set.'))return;
  ...
  try{
    ...
    currentBulkDymoItems=items;
    $('#dymoBulkStatus').textContent='40 secure labels are ready for direct printing or .label download.';
    $('#qrBulkDialog').showModal();
...
$('#downloadAllDymoBtn')?.addEventListener('click',async()=>{
  if(!currentBulkDymoItems.length)return;
  await downloadAllDymoLabels(currentBulkDymoItems);
...
// JSQR_PRODUCTION_BEGIN
async function runJsQrProductionScanner(target,expectedRadioId=null){
  const employeeReturn=target==='employeeReturn';
  const employeeCheckout=target==='employeeCheckout';
  if(typeof globalThis.jsQR!=='function'){
    employeeMessage('QR scanner did not load. Close and reopen Valet Radio HQ, then try again.','error');return;
  }
  if(!navigator.mediaDevices?.getUserMedia){
    employeeMessage('Camera access is unavailable in this browser.','error');return;
  }
  if(employeeCheckout){
    const vm=getEmployeeWorkspace(state,profile);
    if(!vm.canCheckout){employeeMessage('Return your current radio before checking out another.','error');return;}
    if(!selectedShiftCode){employeeMessage('Select your scheduled shift before scanning a radio.','error');return;}
  }

  try{
    dialog.showModal();
    $('#scannerStatus').textContent=employeeReturn?`Scanning ${expectedRadioId} secure QR code…`:'Scanning radio QR code…';
    $('#scannerDiagnostic').textContent='Camera starting…';
    ...
    $('#scannerDiagnostic').textContent='Camera ready';

    const canvas=document.createElement('canvas');
    let busy=false;
    scannerTimer=setInterval(async()=>{
      if(busy||!scannerStream||video.readyState<2)return;
      ...
      const raw=String(result?.data||'').trim();if(!raw)return;

      busy=true;
      $('#scannerStatus').textContent='QR detected — verifying radio…';
      $('#scannerDiagnostic').textContent='Secure QR detected';
      try{
        if(employeeReturn){
          stopScanner();
          await mutate(()=>api.returnRadioSecure(raw),`${expectedRadioId} returned • Secure QR Verified`,{employee:true});
          return;
        }
        if(employeeCheckout){
          stopScanner();
          await mutate(()=>api.checkoutRadioSecure(raw,selectedShiftCode,getShiftWorkDate(selectedShiftCode,new Date())),`Radio checked out • ${shiftLabel(selectedShiftCode)} • Secure QR Verified`,{employee:true});
          return;
        }
      }catch(err){
        busy=false;
        $('#scannerStatus').textContent=humanError(err);
        $('#scannerDiagnostic').textContent='QR verification failed — try again';
      }
    },180);
  }catch(err){
    stopScanner();employeeMessage(cameraErrorMessage(err),'error');
  }
}

async function beginEmployeeJsQrScan(){
  const vm=getEmployeeWorkspace(state,profile);
  if(vm.activeCheckout) return runJsQrProductionScanner('employeeReturn',vm.activeCheckout.radio_id);
  return runJsQrProductionScanner('employeeCheckout');
}
// JSQR_PRODUCTION_END
...
async function openScanner(target,expectedRadioId=null){
  ...
  if(!scannerMode){const action=employeeReturn?'return':employeeCheckout?'check out':'select';report(employeeScan?`Camera QR code scanning is required to ${action} your radio. Open Valet Radio HQ in Safari on iPhone/iPad or Chrome on Android and allow camera access.`:'Camera QR code scanning is unavailable in this browser. Use the radio dropdown.','error');return;}
  ...
  try{
    ...
    const switchToZxing=()=>{
      if(typeof globalThis.jsQR==='function'){scannerMode='jsqr';detector=null;nativeEmptyReads=0;$('#scannerStatus').textContent='Scanning secure radio QR code… Center the QR code in the box.';return true;}
      ...
      $('#scannerStatus').textContent='Scanning secure radio QR code… Center the QR code in the box.';
      return true;
    };
    ...
    scannerTimer=setInterval(async()=>{
      ...
      try{
        ...
        }else if(scannerMode==='jsqr'){
          const value=decodeFrameWithJsQr(video,scanCanvas);
