...
function renderEmployees(){
  if(!isManager(profile))return;const summary=summarizeEmployees(profiles,state.radios);
  const cards=[['Pending approvals',summary.pending],['Active employees',summary.active],['Disabled',summary.disabled],['Rejected',summary.rejected],['Holding radios',summary.holding]];
  $('#employeeSummary').innerHTML=cards.map(([l,v])=>`<div class="employee-summary-card"><span>${l}</span><strong>${v}</strong></div>`).join('');
  const filter=$('#employeeStatusFilter').value||'PENDING';const rows=filterEmployeesByStatus(profiles.filter(p=>p.role!=='MANAGER'),filter);
  $('#employeeRows').innerHTML=rows.map(p=>{const status=p.approval_status||(p.is_active?'ACTIVE':'DISABLED'),radio=currentRadioForProfile(p.id);let actions='';if(status==='PENDING')actions=`<button class="mini-btn primary" data-employee-admin="approve" data-id="${p.id}">Approve</button><button class="mini-btn danger" data-employee-admin="reject" data-id="${p.id}">Reject</button>`;else if(status==='ACTIVE')actions=`<button class="mini-btn" data-employee-admin="history" data-id="${p.id}">View History</button><button class="mini-btn danger" data-employee-admin="disable" data-id="${p.id}" ${radio?'disabled title="Return assigned radio first"':''}>Disable</button>`;else actions=`<button class="mini-btn primary" data-employee-admin="enable" data-id="${p.id}">Restore Access</button>`;return `<tr><td><div class="employee-name-cell"><strong>${escapeHtml(p.display_name)}</strong><span>${escapeHtml(p.email||'No email')}</span></div></td><td>${escapeHtml(p.employee_id)}</td><td>${escapeHtml(p.department)}</td><td>${escapeHtml(p.email||'—')}</td><td><span class="account-badge account-${status}">${approvalLabel(status)}</span></td><td>${radio?`<strong>${radio.id}</strong><br><small>${fmt(radio.checkoutAt)}</small>`:'—'}</td><td>${fmt(p.created_at)}</td><td><div class="employee-actions">${actions}</div></td></tr>`;}).join('')||'<tr><td colspan="8" class="empty-state">No employees match this status.</td></tr>';
}
...
async function mutateEmployee(kind,id){const p=profiles.find(x=>x.id===id);if(!p)return;const labels={approve:'approve',reject:'reject',disable:'disable',enable:'restore access for'};if(!confirm(`${labels[kind]} ${p.display_name}?`))return;const fn={approve:()=>api.approveEmployee(id),reject:()=>api.rejectEmployee(id),disable:()=>api.disableEmployee(id),enable:()=>api.enableEmployee(id)}[kind];if(!fn)return;try{await fn();showToast(`${p.display_name}: account updated`);await loadData({quiet:true});}catch(err){showToast(humanError(err));}}
...
$('#signUpForm').addEventListener('submit',async e=>{e.preventDefault();if(!api){authMessage('Supabase is not configured yet.','error');return;}const displayName=$('#signUpName').value.trim(),employeeId=$('#signUpEmployeeId').value.trim(),department=$('#signUpDepartment').value.trim(),email=$('#signUpEmail').value.trim(),password=$('#signUpPassword').value,confirmPassword=$('#signUpConfirmPassword').value;if(!displayName||!employeeId||!department||!email||!password){authMessage('Complete every field.','error');return;}if(password!==confirmPassword){authMessage('Passwords do not match.','error');return;}authMessage('Creating your account…','success');try{const data=await api.signUpEmployee({email,password,displayName,employeeId,department});e.target.reset();if(data?.session){await establishSession(data.session);}else{setAuthMode('signin');authMessage('Account created. Check your email to verify it, then sign in. A Manager must approve your account before fleet access.','success');}}catch(err){authMessage(humanError(err));}});
...
$('#employeeReturnBtn').addEventListener('click',async()=>{const id=$('#employeeReturnBtn').dataset.id;if(id)await mutate(()=>api.returnRadio(id),`${id} returned successfully`,{employee:true});});
...
async function openScanner(target){scannerTarget=target;if(!canUseBarcodeDetector()||!navigator.mediaDevices?.getUserMedia){(target==='employee'?employeeMessage:showMessage)('Camera scanning is not supported in this browser. Use the radio dropdown.','error');return;}try{dialog.showModal();scannerStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'}});video.srcObject=scannerStream;await video.play();const detector=new BarcodeDetector({formats:['qr_code']});$('#scannerStatus').textContent='Scanning…';scannerTimer=setInterval(async()=>{try{const codes=await detector.detect(video);for(const code of codes){const id=parseRadioCode(code.rawValue);if(!id)continue;const select=scannerTarget==='employee'?$('#employeeRadioSelect'):$('#radioSelect');const opt=[...select.options].find(o=>o.value===id);if(opt){select.value=id;if(scannerTarget==='manager')updateSelectedVisual();$('#scannerStatus').textContent=`Found ${id}`;setTimeout(stopScanner,450);return;}$('#scannerStatus').textContent=`${id} is not eligible for this action.`;}}catch{}},400);}catch{stopScanner();(target==='employee'?employeeMessage:showMessage)('Camera access was unavailable. Use the radio dropdown instead.','error');}}
$('#closeScanner').addEventListener('click',stopScanner);$('#scanSupport').textContent=canUseBarcodeDetector()?'Camera QR scanning is supported in this browser.':'Camera QR scanning may not be supported here; manual selection is available.';$('#scanBtn').addEventListener('click',()=>openScanner('manager'));$('#employeeScanBtn').addEventListener('click',()=>openScanner('employee'));
