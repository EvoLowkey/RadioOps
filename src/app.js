...
import { parseRadioCode, canUseBarcodeDetector, matchesAssignedRadio, getPreferredCameraConstraints } from './scanner.js';
import {
...
async function openScanner(target,expectedRadioId=null){scannerTarget=target;const employeeReturn=target==='employeeReturn';const report=target==='manager'?showMessage:employeeMessage;if(!canUseBarcodeDetector()||!navigator.mediaDevices?.getUserMedia){report(employeeReturn?'Camera QR scanning is required to return your radio. Use a supported mobile browser and allow camera access.':'Camera scanning is not supported in this browser. Use the radio dropdown.','error');return;}try{dialog.showModal();$('#scannerStatus').textContent=employeeReturn?`Allow camera access and scan the QR code on ${expectedRadioId}.`:'Allow camera access, then point at the radio QR code.';scannerStream=await navigator.mediaDevices.getUserMedia(getPreferredCameraConstraints());video.srcObject=scannerStream;await video.play();const detector=new BarcodeDetector({formats:['qr_code']});let scanBusy=false;$('#scannerStatus').textContent=employeeReturn?`Scan the QR code on ${expectedRadioId} to verify the return.`:'Scanning…';scannerTimer=setInterval(async()=>{if(scanBusy)return;try{const codes=await detector.detect(video);for(const code of codes){const id=parseRadioCode(code.rawValue);if(!id)continue;if(employeeReturn){if(!matchesAssignedRadio(id,expectedRadioId)){ $('#scannerStatus').textContent=`Wrong radio scanned. You are assigned ${expectedRadioId}.`; employeeMessage(`Wrong radio scanned. You are assigned ${expectedRadioId}.`,'error'); continue;}scanBusy=true;$('#scannerStatus').textContent=`${id} verified. Completing return…`;stopScanner();await mutate(()=>api.returnRadioVerified(id),`${id} returned • QR Verified Return`,{employee:true});return;}const select=scannerTarget==='employee'?$('#employeeRadioSelect'):$('#radioSelect');const opt=[...select.options].find(o=>o.value===id);if(opt){select.value=id;if(scannerTarget==='manager')updateSelectedVisual();$('#scannerStatus').textContent=`Found ${id}`;setTimeout(stopScanner,450);return;}$('#scannerStatus').textContent=`${id} is not eligible for this action.`;}}catch{}},400);}catch(err){stopScanner();const denied=err?.name==='NotAllowedError'||err?.name==='SecurityError';report(employeeReturn?(denied?'Camera permission is required to return a radio. Enable camera access for RadioOps in your browser settings, then try again.':'Camera could not be started. Use a supported browser with camera access to verify the return.'):'Camera access was unavailable. Use the radio dropdown instead.','error');}}
