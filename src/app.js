...
async function mutateEmployee(kind,id){const p=profiles.find(x=>x.id===id);if(!p)return;const labels={approve:'approve',reject:'reject',disable:'disable',enable:'restore access for',promote:'promote to Manager',demote:'demote from Manager'};if(kind==='remove'){if(!confirm(`Permanently remove ${p.display_name}? Their login will be deleted, their past radio history will be preserved, and they must sign up again to return to RadioOps.`))return;}else if(!confirm(`${labels[kind]} ${p.display_name}?`))return;const fn={approve:()=>api.approveEmployee(id),reject:()=>api.rejectEmployee(id),disable:()=>api.disableEmployee(id),enable:()=>api.enableEmployee(id),promote:()=>api.promoteToManager(id),demote:()=>api.demoteManager(id),remove:()=>api.removeEmployee(id)}[kind];if(!fn)return;try{await fn();showToast(kind==='remove'?`${p.display_name}: removed from RadioOps`:`${p.display_name}: account updated`);await loadData({quiet:true});}catch(err){showToast(humanError(err));}}
