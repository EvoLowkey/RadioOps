...
function showVerifiedLoginNotice(){
  ...
  if(verified){banner.innerHTML='<strong>✓ Email successfully verified</strong><span>Your email has been successfully verified. You can now log in to Valet Ops HQ.</span>';}
  if(passwordReset){banner.innerHTML='<strong>✓ Password updated successfully</strong><span>Your password has been changed. Sign in to Valet Ops HQ with your new password.</span>';}
...
function humanError(err){
  ...
  if(err?.code==='CONFIG_MISSING')return 'Valet Ops HQ needs its Supabase connection configured before sign-in.';
...
function setAuthMode(next){
  ...
  if(forgot){
    ...
    $('#authCopy').textContent='Enter your email and Valet Ops HQ will send a secure password reset link.';
    const current=$('#signInEmail').value.trim();if(current)$('#forgotPasswordEmail').value=current;
  }else{
    $('#authTitle').textContent=signup?'Create your employee account':'Sign in to Valet Ops HQ';
    $('#authCopy').textContent=signup?'Use any email address. Your account stays pending until a Valet Ops HQ Manager approves it.':'Use your workplace account to access RadioOps and keep the fleet synchronized across devices.';
  }
  authMessage('','');
}
...
function renderAccountGate(kind){
  const data={
    ...
    DISABLED:['!','Access disabled','Account access is disabled','Contact a workplace Manager to restore your Valet Ops HQ access.']
...
async function mutateEmployee(kind,id){const p=profiles.find(x=>x.id===id);if(!p)return;const labels={approve:'approve',reject:'reject',disable:'disable',enable:'restore access for',promote:'promote to Manager',demote:'demote from Manager'};if(kind==='remove'){if(!confirm(`Permanently remove ${p.display_name}? Their login will be deleted, their past radio history will be preserved, and they must sign up again to return to Valet Ops HQ.`))return;}else if(!confirm(`${labels[kind]} ${p.display_name}?`))return;const fn={approve:()=>api.approveEmployee(id),reject:()=>api.rejectEmployee(id),disable:()=>api.disableEmployee(id),enable:()=>api.enableEmployee(id),promote:()=>api.promoteToManager(id),demote:()=>api.demoteManager(id),remove:()=>api.removeEmployee(id)}[kind];if(!fn)return;try{await fn();showToast(kind==='remove'?`${p.display_name}: removed from Valet Ops HQ`:`${p.display_name}: account updated`);await loadData({quiet:true});}catch(err){showToast(humanError(err));}}
...
$('#authSignInTab').addEventListener('click',()=>setAuthMode('signin'));$('#authSignUpTab').addEventListener('click',()=>setAuthMode('signup'));$('#forgotPasswordBtn').addEventListener('click',()=>setAuthMode('forgot'));$('#backToSignInBtn').addEventListener('click',()=>setAuthMode('signin'));
