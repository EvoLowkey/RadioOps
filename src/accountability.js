...
export function accountabilityBanner(record){
  ...
  if(status==='RETURN_DUE_SOON')return {tone:'warning',title:'Radio Return Required',message:'Your shift ends in 15 minutes. Please return and scan your assigned radio before clocking out and receiving tips.'};
...
export function shouldNotifyAccountability(record,lastNotifiedStatus){
  const status=record?.computed_return_status||record?.return_status;
  return ['RETURN_DUE_SOON','UNRETURNED_AFTER_SHIFT'].includes(status)&&status!==lastNotifiedStatus;
}
